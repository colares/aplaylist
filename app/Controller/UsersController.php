...
class UsersController extends AppController {
    ...
    public function login() {
		...
		if($this->Session->read('FB.Me')){
			...
			$this->Session->write('Auth.User', $this->Session->read('Auth.User.User'));
			$this->Session->write('Auth.User.Profile', $this->request->data['Profile']);
			$this->Session->setFlash(__('Hi, <strong>%s</strong>! Our fingers are crossed hoping for solving your issues today!', $this->Session->read('Auth.User.Profile.name')), 'default', array('class' => 'alert alert-success'));
			$this->redirect('/profile/issues');
		}
	...
	public function requestResetPassword(){
		if ($this->request->is('post')) {
			if(!$this->Recaptcha->valid($this->request)){
				$this->Session->setFlash(__('The code informed at the end of this form was mistyped!'), 'default', array('class' => 'alert alert-error')); 	
				...
				if(!$this->request->data){
					...
					if(!$this->User->save($updateData)){
						$this->Session->setFlash(__('An error occurred. Please try again or contact the administrator.'), 'default', array('class' => 'alert alert-error'));
					} else {	
						...
						if(!$res['success']){
							$this->Session->setFlash(__('Error occurred while sending an e-mail. Please try again or contact the administrator.'), 'default', array('class' => 'alert alert-error'));
						} else {
	...
	*/
	function resetPassword(){
		if(
			...
			$this->Session->setFlash(__('Invalid Link. Have you entered the right link?'), 'default', array('class' => 'alert alert-error'));
			...
			if(!$user){
				$this->Session->setFlash(__('Invalid Verification Code. Make sure that your code has not expired.'), 'default', array('class' => 'alert alert-error'));
				$this->redirect('/users/requestResetPassword');
			}else{
				...
				if ($this->request->is('post')) {
					if(!$this->Recaptcha->valid($this->request)){
						...
						if ($this->User->save($user)) {
							$this->Session->setFlash(__('Your password has successfully changed!'), 'default', array('class' => 'alert alert-success'));
							$this->redirect('/login');
						}else{
							$this->Session->setFlash(__('Error occurred when chanding your password.'), 'default', array('class' => 'alert alert-error'));
						}//if ($this->User->save($user)) 
