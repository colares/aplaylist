...
class AppController extends Controller {
	 ...
	 * @author Thiago Colares
	 */
	protected function checkOwner(&$options, $modelName = null){
		...
		$this->loadModel('User');
		if(isset($this->User->hasOne[Inflector::singularize($modelName)]['foreignKey'])){
			$field = $this->User->hasOne[Inflector::singularize($modelName)]['foreignKey'];
		} elseif (isset($this->User->hasMany[Inflector::singularize($modelName)]['foreignKey'])) {
			$field = $this->User->hasMany[Inflector::singularize($modelName)]['foreignKey'];
		}
		
		// If there is user_id and your are not admin...
		if(
			$this->{Inflector::singularize($modelName)}->hasField($field) 
			&&
			$this->Session->read('Auth.User.role') != 'admin'			
		){
			// You better be owner or beat it!
			$options['conditions'][Inflector::singularize($modelName) . '.' . $field] = $this->Session->read('Auth.User.id');
			return;
		}	
	}
...
//     }
	...
	public function admin_index(){
		...
		if($res['success']){
			...
			$this->Session->setFlash(__('Error:') . ' ' . $res['message'], 'default', array('class' => 'alert alert-error'));
				...
				*/
				...
				if($this->res['success'] == true){
					if(isset($this->successMessage)){
						$this->Session->setFlash($this->successMessage, 'default', array('class' => 'alert alert-success'));
					} else {
						$this->Session->setFlash(__("%s has been successfully added.", $this->title), 'default', array('class' => 'alert alert-success'));
					}
                    ...
                    $this->Session->setFlash(__('Error while saving') . ' ' . $this->title . '. ' . $this->res['message'], 'default', array('class' => 'alert alert-error'));
                }


            }
        } else {
            $this->Session->setFlash(__('Error while loading form') . ' ' . $this->title . '. ' . $this->res['message'], 'default', array('class' => 'alert alert-error'));
            /**
	...
	*/
	...
	function admin_edit($id = null) {
		...
		if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if($res['success'] == true){
				$this->Session->setFlash(__("Changes has been successfully saved.", $this->title), 'default', array('class' => 'alert alert-success'));
				if(empty($this->redirectURLHandler)){
					$this->redirect(array('action' => 'index'));
				} else {
					$this->redirect($this->redirectURLHandler);
                ...
                $this->Session->setFlash(__('Error while saving',true) . ' ' . $this->title . '<br>' . $res['message'], 'default', array('class' => 'alert alert-error'));
            }
		...
		// Read action
			...
			if($res['success'] != true){
                $this->Session->setFlash(__('Error reading item',true) . ' ' . $this->title . '<br>' . $res['message'], 'default', array('class' => 'alert alert-error'));
				if(empty($this->redirectURLHandler)){
					$this->redirect(array('action' => 'index'));
				} else {
					$this->redirect($this->redirectURLHandler);
	...
	function admin_delete($id = null) {
        ...
        if($res['success'] == true){
			...
			*/
			...
			if (empty($id)) {
			    $this->Session->setFlash(__('ID can not be null.'), 'default', array('class' => 'alert alert-error'));
			...
			*/
			if ($this->{Inflector::singularize($this->name)}->delete($id)) {
			    $this->Session->setFlash(__('Item has been successfully deleted.'), 'default', array('class' => 'alert alert-success'));
			    $this->redirect($redUrl);
			}

			// Ops! Error!
		    $this->Session->setFlash(__('Item was no deleted. Error while deleting item.'), 'default', array('class' => 'alert alert-error'));

        } else {
            $this->Session->setFlash(__('Error while deleting',true) . ' ' . $this->title . '<br>' . $res['message'], 'default', array('class' => 'alert alert-error'));
        }
		/*
			TODO Show errors
		*/
